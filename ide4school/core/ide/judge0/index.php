<?php
$active_compiler_languages = $db->getActiveCompilerLanguages();
?><!DOCTYPE html>
<html>
...
<head>
    ...
    <title><?php if(isset($file_name)) {echo $file_name;} else { echo 'Neue Datei';}?>  - ide4school - Entwicklungsumgebung</title>
    <meta property="og:title" content="<?php if(isset($file_name)) {echo $file_name;} else { echo 'Neue Datei';}?>  - ide4school - Entwicklungsumgebung">
    ...
    <script type="text/javascript" src="core/ide/judge0/third_party/download.js"></script>



<link type="text/css" rel="stylesheet" href="core/ide/judge0/css/ide.css">
    ...
    <script>
        var defaultUrl = localStorageGetItem("api-url") || "http://api.ide4school.com:2358";
...
var layoutConfig = {
    ...
    content: [{
        ...
        content: [{
            ...
            id: "source",
            title: "<?php if(isset($file_name)) {echo $file_name;} else { echo 'Neue Datei';}?>",
            isClosable: false,
            componentState: {
                readOnly: false
            }
...
function handleResult(data) {
    ...
    console.log("Es dauerte " + (timeEnd - timeStart) + " ms um deinen Code auszuführen und ein Ergebnis zu erhalten.");
...
function run() {
    if (sourceEditor.getValue().trim() === "") {
        showError("Fehler", "Code kann nicht leer sein!");
        return;
    } else {
    ...
    var sendRequest = function(data) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                console.log(`Dein Anfragetoken ist: ${data.token}`);
                if (wait == true) {
                    handleResult(data);
                } else {
    ...
    function insertTemplate() {
        <?php
            if(isset($file_content)) {
                echo 'sourceEditor.setValue(`' . $file_content . '`);';
            }
            else {
                echo 'sourceEditor.setValue(``);';
            }
        ?>
...
$(document).ready(function () {
    ...
    console.log("ide4school Code Editor - developed by em CLOUDsolutions");
    ...
    $insertTemplateBtn.click(function (e) {
        if (isEditorDirty && confirm("Sind Sie sicher? Ihre aktuellen Änderungen gehen verloren.")) {
            insertTemplate();
        }
    });
...
<body>
    <div id="site-navigation" class="ui small inverted menu">
        <div id="site-header" class="header item">
            <a href="#">
                <img id="site-icon" src="core/ide/judge0/images/judge0_icon.png">
                <h2>ide4school IDE</h2>
            </a>
        </div>
        <div class="left menu">

            <div class="link item" onclick="$('#site-settings').modal('show')"><i class="cog icon"></i> Einstellungen</div>
            <div class="item borderless">
                <select id="select-language" class="ui dropdown">
                    <?php
                    foreach($active_compiler_languages as $compiler_language) {
                        echo'<option value="'.$compiler_language['language_id'].'" mode="'.$compiler_language['mode'].'">'.$compiler_language['name'].'</option>';
                    }
                    ?>
                </select>
            </div>
            <div class="item fitted borderless wide screen only">
                <div class="ui input">
                    <input id="compiler-options" type="text" placeholder="Compiler Optionen"></input>
                </div>
            </div>
            <div class="item borderless wide screen only">
                <div class="ui input">
                    <input id="command-line-arguments" type="text" placeholder="Argumente für die Befehlszeile"></input>
                </div>
            </div>
            <div class="item no-left-padding borderless">
                <button OnClick="saveFile()"  class="ui primary labeled icon button"><i class="save icon"></i>Speichern</button>
            </div>
            <div class="item no-left-padding borderless">
                <button id="run-btn" class="ui primary labeled icon button"><i class="play icon"></i>Ausführen (F9)</button>
            </div>
    ...
    <div id="site-footer">
        ...
        <span><a href="https://em-cloud-solutions.de">ide4school Entwicklungsumgebung</a>
    ...
    <script>
        function saveFile(){
            document.getElementById("file_content").value = sourceEditor.getValue();
    ...
    <script>
        function saveFile(){
            ...
            document.getElementById("file_content").value = sourceEditor.getValue();
            document.getElementById("file_name").value = prompt("Bitte den Namen der neuen Datei MIT der Dateiendung eingeben. Diese wird anschließend in deinem Benutzerordner gespeichert.");
            if(document.getElementById("file_name").value == "") {
                document.getElementById("file_name").value = "Neue Datei vom "+current_timestamp+" Uhr.txt";
            }
            document.getElementById("saveFileForm").submit();        
        }
    </script>';
