...
$user_dir = $db->getMyUserDir();

$class_dir = $db->getClassDirViaName($getCurrentUserData['class']);

if(isset($_GET['drive'])) {
    $_SESSION['selected_drive_fm_internal'] = $_GET['drive'];
}

//Überprüfe ob von jedem Benutzer ein Benutzerordner und ein Abgabeordner existiert und lege ihn notfalls an
$users = $db->getAllUsers();
foreach($users as $user) {
    if(!file_exists('../../files/users/' . $user['username'])) {
        mkdir('../../files/users/' . $user['username'], 0777, true);
    }
    if(!file_exists('../../files/submissions/' . $user['username'])) {
        mkdir('../../files/submissions/' . $user['username'], 0777, true);
    }
}

//Das gleiche für alle Klassen, nur werden Leerzeichen mit Unterstrichen ersetzt
$classes = $db->getAllClasses();
foreach($classes as $class) {
    if(!file_exists('../../files/classes/' . str_replace(" ", "_", $class['name']))) {
        mkdir('../../files/classes/' . str_replace(" ", "_", $class['name']), 0777, true);
    }
}

//Projekt

$project_data = $db->getProjectData($_SESSION['fm_internal_security_project_ident']);
...
//Application Title
define('APP_TITLE', 'ide4school Filemanager');
...
if($_SESSION['selected_drive_fm_internal'] == "class") {
    $root_path = '../../' . $class_dir;
}
if($_SESSION['selected_drive_fm_internal'] == "my") {
    $root_path = '../../' . $user_dir;
}
if($_SESSION['selected_drive_fm_internal'] == "ad-users" && $getCurrentUserData['role'] != "Schüler") {
    $root_path = '../../files/users';
}
if($_SESSION['selected_drive_fm_internal'] == "ad-classes" && $getCurrentUserData['role'] != "Schüler") {
    $root_path = '../../files/classes';
}
if($_SESSION['selected_drive_fm_internal'] == "ad-submissions" && $getCurrentUserData['role'] != "Schüler") {
    $root_path = '../../files/submissions';
}
if($_SESSION['selected_drive_fm_internal'] == "project") {
    $root_path = '../../' . $project_path;
}
if($_SESSION['selected_drive_fm_internal'] == "exam-projects") {
    $root_path = '../../files/projects/' . $getCurrentUserData['username'];
}
if($_SESSION['selected_drive_fm_internal'] == "exam-my-folder") {
    $root_path = '../../' . $user_dir;
}
                ...
                <div class="row justify-content-md-center h-100">
                    <div class="card-wrapper">
                            ...
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="form-group">
                                       <div class="brand">
                                            <svg version="1.0" xmlns="http://www.w3.org/2000/svg" M1008 width="100%" height="80px" viewBox="0 0 238.000000 140.000000" aria-label="ide4school Filemanager">
                        ...
                        <div class="footer text-center">
                            &mdash;&mdash; &copy;
                            <a href="https://ide4school.de/features/filemanager" target="_blank" class="text-muted" data-version="<?php echo VERSION; ?>">CCP Programmers</a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
...
if($_SESSION['disk_state'] == "writable") {
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        if ($write_results === false){
            ...
            die("Schreiben in Datei nicht möglich! Rechte/Benutzer überprüfen!");
        }
        die(true);
    }
    ...
    if (isset($_POST['type']) && $_POST['type'] == "backup" && !empty($_POST['file'])) {
        ...
        $date = date("dMy-His");
        $newFileName = "{$fileName}-{$date}.backup";
        ...
        try {
            ...
            if (copy($fullyQualifiedFileName, $fullPath . $newFileName)) {
                echo "Sicherungskopie {$newFileName} erstellt";
            } else {
                throw new Exception("Konnte Datei {$fileName} nicht kopieren!");
            }
        } catch (Exception $e) {
            echo $e->getMessage();
        }
    ...
    if(isset($_POST['type']) && $_POST['type'] == "upload" && !empty($_REQUEST["uploadurl"])) {
        ...
        if(!$isFileAllowed) {
            $err = array("message" => "Dateiendung ist nicht erlaubt.");
            event_callback(array("fail" => $err));
            exit();
        }
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (FM_PATH != '') {
            ...
            // Pfad zum Submissions-Verzeichnis
            $submissionsPath = '../../files/submissions';

            // Pfad zum Classes-Verzeichnis
            $classesPath = '../../files/classes';

            // Überprüfen, ob das zu löschende Verzeichnis ein direkter Unterordner von "users", "submissions" oder "classes" ist
            $isDirectSubfolderUsers = is_dir($usersPath . '/' . $del);
            $isDirectSubfolderSubmissions = is_dir($submissionsPath . '/' . $del);
            $isDirectSubfolderClasses = is_dir($classesPath . '/' . $del);

            // Überprüfen, ob das zu löschende Verzeichnis ein Unterordner eines Unterordners von "users", "submissions" oder "classes" ist
            $isSubfolder = false;
            if (!$isDirectSubfolderUsers && !$isDirectSubfolderSubmissions && !$isDirectSubfolderClasses) {
                $iteratorUsers = new RecursiveIteratorIterator(
                    new RecursiveDirectoryIterator($usersPath, RecursiveDirectoryIterator::SKIP_DOTS),
                    RecursiveIteratorIterator::SELF_FIRST
                );

                $iteratorSubmissions = new RecursiveIteratorIterator(
                    new RecursiveDirectoryIterator($submissionsPath, RecursiveDirectoryIterator::SKIP_DOTS),
                    RecursiveIteratorIterator::SELF_FIRST
                );

                $iteratorClasses = new RecursiveIteratorIterator(
                    new RecursiveDirectoryIterator($classesPath, RecursiveDirectoryIterator::SKIP_DOTS),
                    RecursiveIteratorIterator::SELF_FIRST
                );

                foreach ($iteratorUsers as $fileInfo) {
                    if ($fileInfo->isDir() && $fileInfo->getBasename() === $del) {
                        $isSubfolder = true;
                        break;
                    }
                }
                ...
                if (!$isSubfolder) {
                    foreach ($iteratorSubmissions as $fileInfo) {
                        if ($fileInfo->isDir() && $fileInfo->getBasename() === $del) {
                            $isSubfolder = true;
                            break;
                        }
                ...
                if (!$isSubfolder) {
                    foreach ($iteratorClasses as $fileInfo) {
                        if ($fileInfo->isDir() && $fileInfo->getBasename() === $del) {
                            $isSubfolder = true;
                            break;
                        }
            ...
            if ($isDirectSubfolderUsers || $isDirectSubfolderSubmissions || $isDirectSubfolderClasses) {
                fm_set_msg(lng('user_folder_cannot_be_deleted'), 'error');
                fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
            } 
                else {
...
if (isset($_POST['group'], $_POST['delete']) && !FM_READONLY) {
    ...
    if (is_array($files) && count($files)) {
        foreach ($files as $f) {
            if ($f != '') {
                $del = str_replace('/', '', fm_clean_path($f));
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p><h3><a href="https://ide4school.de/features/filemanager" target="_blank" class="app-v-title"> ide4school Filemanager <?php echo VERSION; ?></a></h3></p>
                        <p>Autor: Elias Müller & Prasath Mani</p>
                        <p>E-Mail: <a href="mailto:help@ide4school.de">help@ide4school.de</a> </p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><a href="https://ide4school.de/features/filemanager" target="_blank"><i class="fa fa-question-circle"></i> <?php echo lng('Help Documents') ?> </a> </li>
                                <li class="list-group-item"><a href="https://ide4school.de/features/filemanager" target="_blank"><i class="fa fa-bug"></i> <?php echo lng('Report Issue') ?></a></li>
    ...
    <div class="row">
        <div class="col-12">
            <?php if(!$quickView) { ?>
                ...
                <p class="break-word">
                    Pfad: <?php echo fm_enc(fm_convert_win($file_path)) ?><br>
                    Dateigröße: <?php echo ($filesize_raw <= 1000) ? "$filesize_raw bytes" : $filesize; ?><br>
                    ...
                    if (($is_zip || $is_gzip) && $filenames !== false) {
                        ...
                        Gesamtgröße: <?php echo fm_get_filesize($total_uncomp) ?><br>
                        Größe in Archiven: <?php echo fm_get_filesize($total_comp) ?><br>
                        Kompression: <?php echo round(($total_comp / max($total_uncomp, 1)) * 100) ?>%<br>
                        <?php
                    }
                    // Image info
                    if ($is_image) {
                ...
                <p>
                    ...
                    <b><a href="#" OnClick="openIDE('<?php echo trim($file_path, "../../");?>')"><i class="fa fa-external-link-square"></i> <?php echo 'In der Entwicklungsumgebung öffnen' ?></a></b>
                    &nbsp;
                    <?php
                    // ZIP actions
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-right pt-1">
                ...
                <?php if ($is_text) { ?>
                    <?php if ($isNormalEditor) { ?>
                        ...
                        <button type="button" class="btn btn-sm btn-outline-primary" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i> Speichern
                        </button>
                    <?php } else { ?>
...
<form action="" method="post" class="pt-3">
        ...
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <a title="<?php echo 'In der Entwicklungsumgebung öffnen' ?>" href="#" OnClick="openIDE('<?php echo trim($path, "../../") . "/" . $f;?>')"><i class="fa fa-terminal"></i></a>
                        ...
                        <?php  endif; ?>
                        <?php if($_SESSION['selected_drive_fm_internal'] == "my" && $db->isSubmissionFunktionEnabled()) { ?><a title="<?php echo 'Abgeben' ?>" href="#" onclick="createSubmission('<?=$getCurrentUserData['username'] . '/' . $f?>')"><i class="fa fa-gavel"></i></a><?php } ?>
    ...
    <div class="row">
        ...
        <div class="col-3 d-none d-sm-block"><a href="https://ide4school.de/features/filemanager" target="_blank" class="float-right text-muted">ide4school Filemanager <?php echo VERSION; ?></a></div>
        <?php else: ?>
            <div class="col-12"><a href="https://ide4school.de/features/filemanager" target="_blank" class="float-right text-muted">ide4school Filemanager <?php echo VERSION; ?></a></div>
        <?php endif; ?>
    </div>
 ...
 class FM_Config
{
    ...
    function __construct()
    {
        ...
        if (strlen($CONFIG)) {
            ...
            $msg = 'ide4school Filemanager<br>Error: Cannot load configuration';
            if (substr($fm_url, -1) == '/') {
                $fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
...
<head>
    ...
    <meta name="description" content="Web based File Manager for ide4school - ide4school Filemanager">
    <meta name="author" content="Elias Müller - Origin by TinyFileManager (https://github.com/prasathmani/tinyfilemanager)">
...
<head>
    ...
    <?php endif; ?>
    <script>
        function createSubmission(filepath){
            document.getElementById("submission_file_path").value = filepath;
            if(document.getElementById("submission_name").value = window.prompt("Name der Abgabe eingeben (z.B. Abgabe der Aufgabe vom 21.05)")) {
                if(document.getElementById("submission_name").value == "") {
                document.getElementById("submission_name").value = "Neue Abgabe"; 
                }
                document.getElementById("createSubmissionForm").submit();
            }
        }

        function openIDE(filepath){
            document.getElementById("filepath").value = filepath;
            document.getElementById("openIDEForm").submit();
        }
    </script>
    <style>
...
<form action="../../submissions" target="_parent" method="POST" id="createSubmissionForm">
            <input name="submission_file_path" type="text" hidden id="submission_file_path">
            <input name="submission_name" type="text" hidden id="submission_name">
            <input name="createSubmission" type="text" hidden id="createSubmission">
        </form>

        <form action="../../ide" target="_parent" method="POST" id="openIDEForm">
            <input name="filepath" type="text" hidden id="filepath">
            <?php
            if($_SESSION['selected_drive_fm_internal'] == "exam-projects" || $_SESSION['selected_drive_fm_internal'] == "exam-my-folder") {
                echo '<input name="return_path" type="text" value="exam&token='.$_SESSION['exam_token_identifier'].'" hidden id="return_path">';
                echo '<input name="read_only" type="text" hidden id="read_only">';
            }
            if(!is_null($_SESSION['fm_internal_security_project_ident']) && $_SESSION['selected_drive_fm_internal'] != "exam-projects" || $_SESSION['fm_internal_security_project_ident'] && $_SESSION['selected_drive_fm_internal'] != "exam-my-folder") {
                echo '<input name="return_path" type="text" value="project&id='.$_SESSION['fm_internal_security_project_ident'].'" hidden id="return_path">';
            }
            ?>
            <input name="openIDE" type="text" hidden id="openIDE">
        </form>
            ...
            <div class="modal-content <?php echo fm_get_theme(); ?>">
                ...
                <div class="modal-body">
                    ...
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="customRadioInline1" name="newfile" value="file" class="custom-control-input" checked="">
          ...
          <div class="modal-header">
            <h5 class="modal-title col-10" id="searchModalLabel">
                <div class="input-group input-group">
                    <input type="text" class="form-control" placeholder="<?php echo lng('Search') ?> nach Dateien und Ordnern" aria-label="<?php echo lng('Search') ?>" aria-describedby="search-addon3" id="advanced-search" autofocus required>
                    <div class="input-group-append">
...
<script>
    ...
    function rename(e, t) {var n = prompt("Neuer Name:", t);null !== n && "" !== n && n != t && (window.location.search = "p=" + encodeURIComponent(e) + "&ren=" + encodeURIComponent(t) + "&to=" + encodeURIComponent(n))}
        ...
        if (typeof n !== 'undefined' && n !== null) {
            if (true) {
                ...
                $.ajax({
                    ...
                    //dataType: "json",
                    success: function(mes){toast("Erfolgreich gespeichert!"); window.onbeforeunload = function() {return}},
        ...
        if(!!searchTxt && searchTxt.length > 2 && path) {
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    } else { searchWrapper.html('<p class="m-2">Keine Ergebnisse!<p>'); }
                },
...
function lng($txt) {
    ...
    // English Language
    $tr['en']['AppName']        = 'ide4school Filemanager';                 $tr['en']['AppTitle']           = 'Dateien';
    $tr['en']['Login']          = 'Anmelden';                               $tr['en']['Username']           = 'Benutzername';
    $tr['en']['Password']       = 'Passwort';                               $tr['en']['Logout']             = 'Abmelden';
    $tr['en']['Move']           = 'Verschieben';                            $tr['en']['Copy']               = 'Kopieren';
    $tr['en']['Save']           = 'Speichern';                              $tr['en']['SelectAll']          = 'Alles auswählen';
    $tr['en']['UnSelectAll']    = 'Nichts auswählen';                       $tr['en']['File']               = 'Datei';
    $tr['en']['Back']           = 'Zurück';                                 $tr['en']['Size']               = 'Größe';
    $tr['en']['Perms']          = 'Berechtigungen';                         $tr['en']['Modified']           = 'Zuletzt geändert';
    $tr['en']['Owner']          = 'Besitzer';                               $tr['en']['Search']             = 'Suche';
    $tr['en']['NewItem']        = 'Neu';                                    $tr['en']['Folder']             = 'Ordner';
    $tr['en']['Delete']         = 'Löschen';                                $tr['en']['Rename']             = 'Umbenennen';
    $tr['en']['CopyTo']         = 'Kopieren nach';                          $tr['en']['DirectLink']         = 'Direkter Link';
    $tr['en']['UploadingFiles'] = 'Dateien hochladen';                      $tr['en']['ChangePermissions']  = 'Berechtigungen ändern';
    $tr['en']['Copying']        = 'Kopieren';                               $tr['en']['CreateNewItem']      = 'Neu erstellen';
    $tr['en']['Name']           = 'Name';                                   $tr['en']['AdvancedEditor']     = 'Erweiterter Editor';
    $tr['en']['RememberMe']     = 'Merken';                                 $tr['en']['Actions']            = 'Aktionen';
    $tr['en']['Upload']         = 'Hochladen';                              $tr['en']['Cancel']             = 'Abbrechen';
    $tr['en']['InvertSelection']= 'Auswahl umkehren';                       $tr['en']['DestinationFolder']  = 'Zielordner';
    $tr['en']['ItemType']       = 'Typ';                                    $tr['en']['ItemName']           = 'Name';
    $tr['en']['CreateNow']      = 'Jetzt erstellen';                        $tr['en']['Download']           = 'Download';
    $tr['en']['Open']           = 'Öffnen';                                 $tr['en']['UnZip']              = 'Entpacken';
    $tr['en']['UnZipToFolder']  = 'In Ordner entpacken';                    $tr['en']['Edit']               = 'Bearbeiten';
    $tr['en']['NormalEditor']   = 'Normaler Editor';                        $tr['en']['BackUp']             = 'Sicherung anlegen';
    $tr['en']['SourceFolder']   = 'Erweiterter Editor';                     $tr['en']['Files']              = 'Dateien';
    $tr['en']['Move']           = 'Verschieben';                            $tr['en']['Change']             = 'Ändern';
    $tr['en']['Settings']       = 'Einstellungen';                          $tr['en']['Language']           = 'Sprache';
    $tr['en']['Folder is empty']    = 'Ordner ist leer';                    $tr['en']['PartitionSize']      = 'Festplattengröße';
    $tr['en']['ErrorReporting'] = 'Fehlerausgabe';                          $tr['en']['ShowHiddenFiles']    = 'Versteckte Dateien anzeigen';
    $tr['en']['Help']               = 'Hilfe';                              $tr['en']['Created']            = 'Erstellt';
    $tr['en']['Free of']        = 'Frei von';                               $tr['en']['Preview']            = 'Vorschau';
    $tr['en']['Help Documents'] = 'Dokumentation';                          $tr['en']['Report Issue']       = 'Fehler melden';
    $tr['en']['Generate']       = 'Generieren';                             $tr['en']['FullSize']           = 'Gesamtgröße';
    $tr['en']['FreeOf']         = 'frei von';                               $tr['en']['CalculateFolderSize']= 'Ordnergröße berechnen';
    $tr['en']['HideColumns']    = 'Berechtigungen/Besitzer verbergen';      $tr['en']['You are logged in']  = 'Du bist angemeldet';
    $tr['en']['Check Latest Version'] = 'Nach Updates suchen';              $tr['en']['Generate new password hash'] = 'Neuen Passwort Hash generieren';
    $tr['en']['Login failed. Invalid username or password'] = 'Anmeldungen fehlgeschlagen. Bitte Benutzername und Passwort überprüfen.';
    $tr['en']['password_hash not supported, Upgrade PHP version'] = 'password_hash Funktion nicht unterstützt. Bitte aktualisiere deine PHP Version.';

    // new - novos

    $tr['en']['Advanced Search']    = 'Erweiterte Suche';           $tr['en']['Error while copying from']    = 'Fehler beim kopieren von';
    $tr['en']['Nothing selected']   = 'Nichts ausgewählt';          $tr['en']['Paths must be not equal']    = 'Pfade dürfen nicht gleich sein';
    $tr['en']['Renamed from']       = 'Umbenannt von';              $tr['en']['Archive not unpacked']       = 'Archiv nicht entpackt';
    $tr['en']['Deleted']            = 'gelöscht';                   $tr['en']['Archive not created']        = 'Archiv nicht erstellt';
    $tr['en']['Copied from']        = 'Kopiert von';                $tr['en']['Permissions changed']        = 'Berechtigungen geändert';
    $tr['en']['to']                 = 'nach';                       $tr['en']['Saved Successfully']         = 'Speichern abgeschlossen';
    $tr['en']['not found!']         = 'nicht gefunden!';            $tr['en']['File Saved Successfully']    = 'Datei erfolgreich gespeichert';
    $tr['en']['Archive']            = 'Archiv';                     $tr['en']['Permissions not changed']    = 'Berechtigungen nicht geändert';
    $tr['en']['Select folder']      = 'Ordner auswählen';           $tr['en']['Source path not defined']    = 'Quellpfad nicht angegeben';
    $tr['en']['already exists']     = 'existiert bereits';          $tr['en']['Error while moving from']    = 'Fehler beim verschieben von';
    $tr['en']['Create archive?']    = 'Archiv erstellen?';          $tr['en']['Invalid file or folder name']    = 'Fehlerhafter Datei-/Ordnername';
    $tr['en']['Archive unpacked']   = 'Archiv entpackt';          $tr['en']['File extension is not allowed']  = 'Dateiendung ist nicht erlaubt';
    $tr['en']['Root path']          = 'Stammpfad';                  $tr['en']['Error while renaming from']  = 'Fehler beim umbenennen von';
    $tr['en']['File not found']     = 'Datei nicht gefunden';       $tr['en']['Error while deleting items'] = 'Fehler beim Löschen';
    $tr['en']['Invalid characters in file name']                = 'Nicht erlaubte Zeichen im Dateinamen';
    $tr['en']['FILE EXTENSION HAS NOT SUPPORTED']               = 'DATEIENDUNG WIRD NICHT UNTERSTÜTZT';
    $tr['en']['Selected files and folder deleted']              = 'Ausgewählte Dateien und Ordner gelöscht';
    $tr['en']['Error while fetching archive info']              = 'Fehler vom Abrufen von Archivinformationen';
    $tr['en']['Delete selected files and folders?']             = 'Ausgewählte Dateien und Ordner löschen?';
    $tr['en']['Search file in folder and subfolders...']        = 'Suche Datei in Ordnern und Unterordnern';
    $tr['en']['Access denied. IP restriction applicable']       = 'Zugriff verweigert. IP-Sperre aktiviert.';
    $tr['en']['Invalid characters in file or folder name']      = 'Nicht erlaubte Zeichen im Datei-/Ordnernamen';
    $tr['en']['Operations with archives are not available']     = 'Operationen mit Archiven sind nicht erlaubt';
    $tr['en']['File or folder with this path already exists']   = 'Datei oder Ordner mit dem gleichen Pfad existiert bereits';
    $tr['en']['user_folder_cannot_be_deleted']                  = 'Benutzer-, Klassen- oder Abgabeordner können nicht gelöscht werden!';

    $tr['en']['Moved from']                 = 'Verschoben von';
